...
function update()
{
    ...
    if [ "$lastterminating" != "$terminating" ]; then
        lastterminating=$terminating
        doprint=1
    fi
...
function printupdate()
{
    if [ "$doprint" -eq "1" ]; then
        ...
        printf "$(date) 🔹 "
        printf "Pods: % 4d (% 4d ⏳️, % 4d init, % 4d 🏃, % 4d ✅, % 4d terminating)" $podcount $pending $init $running $completed $terminating
